...
class FeatureStatistics:
    def __init__(self):
        ...
        feature_dict_list = ["f100", "f101", "f102", "f103", "f104", "f105", "f106", "f107", "fcap", "fnum",
                             "fapost", "fprevapost"]  # the feature classes used in the code
    ...
    def get_word_tag_pair_count(self, file_path) -> None:
        ...
        with open(file_path) as file:
            for line in file:
                ...
                pre_prev_word, pre_prev_tag = sentence[0]
                prev_word, prev_tag = sentence[1]
                cur_word, cur_tag = sentence[2]
                for word_idx in range(2, len(split_words) - 1):
                    next_word, next_tag = sentence[word_idx + 1]
                    ...
                    if "'" in cur_word or '`' in cur_word:
                        if cur_tag not in self.feature_rep_dict["fapost"]:
                            self.feature_rep_dict["fapost"][cur_tag] = 1
                        else:
                            self.feature_rep_dict["fapost"][cur_tag] += 1
                    if "'" in prev_word or '`' in prev_word:
                        if cur_tag not in self.feature_rep_dict["fprevapost"]:
                            self.feature_rep_dict["fprevapost"][cur_tag] = 1
                        else:
                            self.feature_rep_dict["fprevapost"][cur_tag] += 1
                    # fcap
                    if cur_word[0].isupper():
                        if cur_tag not in self.feature_rep_dict["fcap"]:
                            self.feature_rep_dict["fcap"][cur_tag] = 1
                        else:
                            self.feature_rep_dict["fcap"][cur_tag] += 1
                    # fnum
                    if is_number_repl_isdigit(cur_word):
                        if cur_tag not in self.feature_rep_dict["fnum"]:
                            self.feature_rep_dict["fnum"][cur_tag] = 1
                        else:
                            self.feature_rep_dict["fnum"][cur_tag] += 1

                    # Extract all spelling features
                    for i in range(1, min(4, len(cur_word) - 1)):
                        ...
                        if (cur_word[:i], cur_tag) not in self.feature_rep_dict["f102"]:
                            ...
                            self.feature_rep_dict["f102"][(cur_word[:i], cur_tag)] += 1
                    # f103
                    if (pre_prev_tag, prev_tag, cur_tag) not in self.feature_rep_dict["f103"]:
                        self.feature_rep_dict["f103"][(pre_prev_tag, prev_tag, cur_tag)] = 1
                    else:
                        self.feature_rep_dict["f103"][(pre_prev_tag, prev_tag, cur_tag)] += 1
                    # f104
                    ...
                    if (prev_word, cur_tag) not in self.feature_rep_dict["f106"]:
                        ...
                        self.feature_rep_dict["f106"][(prev_word, cur_tag)] += 1
                    # f107
                    if (cur_tag, next_word) not in self.feature_rep_dict["f107"]:
                        self.feature_rep_dict["f107"][(cur_tag, next_word)] = 1
                    else:
                        self.feature_rep_dict["f107"][(cur_tag, next_word)] += 1
                    # f100
                    if (cur_word, cur_tag) not in self.feature_rep_dict["f100"]:
                        ...
                        self.feature_rep_dict["f100"][(cur_word, cur_tag)] += 1
                    pre_prev_word, pre_prev_tag = prev_word, prev_tag
                    prev_word, prev_tag = cur_word, cur_tag
...
def represent_input_with_features(history: Tuple, dict_of_dicts: Dict[str, Dict[Tuple[str, str], int]])\
    ...
    """
    # p_word, p_tag, pp_word, pp_tag = None, None, None, None
    ...
    if "'" in c_word:
        if c_tag in dict_of_dicts["fapost"]:
            features.append(dict_of_dicts["fapost"][c_tag])
    if "'" in p_word:
        if p_tag in dict_of_dicts["fprevapost"]:
            features.append(dict_of_dicts["fprevapost"][p_tag])
    return features
...
def read_test(file_path, tags, tagged=True, skip_tags=False) -> List[Tuple[List[str], List[str]]]:
    """
    reads a test file
        ...
        for line in f:
            ...
            for word_idx in range(len(split_words)):
                if tagged:
                    ...
                    if skip_tags and cur_tag not in tags:
                        skip = True
                else:
                    cur_word, cur_tag = split_words[word_idx], ""
                sentence[WORD].append(cur_word)
                sentence[TAG].append((cur_tag if cur_tag in tags else 'PRP'))
