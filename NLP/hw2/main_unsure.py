...
def create_sentence_list(train=True):
...
def create_sentence_batches(train=True):
    pretrained_glove = gensim.downloader.load(f'glove-twitter-{EMBED_SIZE}')
    batches = {}
    batch_label_dict = {}
...
def train_FF():
    ...
    test_loader = DataLoader(test_set, batch_size=2 * batch_size, shuffle=False, num_workers=2,
                             persistent_workers=True, pin_memory=True)
